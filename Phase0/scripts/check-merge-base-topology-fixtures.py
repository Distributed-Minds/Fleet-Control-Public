...
def main():
    ...
    for case in cases:
        ...
        if "same_set_as" in case:
            other = by_name[case["same_set_as"]]
            if set_identity(case) != set_identity(other):
                errors.append(f"{case['name']}: canonical base-set identity changed under reordering")
        if "same_computation_as" in case:
            other = by_name[case["same_computation_as"]]
            if computation_identity(case) != computation_identity(other):
                errors.append(f"{case['name']}: computation identity changed under equivalent reordering")
        if "different_computation_from" in case:
            other = by_name[case["different_computation_from"]]
            if computation_identity(case) == computation_identity(other):
