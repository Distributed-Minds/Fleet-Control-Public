...
> **Superseded for current installation guidance.** This file preserves the v0.1.0 preview as historical evidence. For new/current installations, use `RELEASE-NOTES-v0.1.1-preview.md`, `GETTING-STARTED.md`, `TROUBLESHOOTING.md`, and `Phase0/95-GITHUB-CAPABILITY-ACCEPTANCE.md`.
>
> Product-surface and GitHub-capability wording below describes the v0.1.0-era snapshot. It is not a universal current statement about interactive or scheduled execution. Current mutation-capable setup requires context-specific capability, stable lineage, current bounded authority, exact-resource evidence, and repository-policy evidence under `Phase0/95-GITHUB-CAPABILITY-ACCEPTANCE.md`.
>
